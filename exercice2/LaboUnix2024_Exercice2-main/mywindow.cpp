...
void MyWindow::on_pushButtonLogin_clicked()
{
  ...
  else 
  {
    if(presence > 0)
    {
      int mdp = verifieMotDePasse(presence, motDePasse);
...
{
  UTILISATEUR listeU[100];
  int nbr = listeUtilisateurs(listeU);
  int i = 0;

  videTableUtilisateurs();
  if(nbr == -1)
  {
    setResultat("Liste vide !");
  }
  else{
    while(i < nbr)
    {
      ajouteTupleTableUtilisateurs(listeU[i].nom, listeU[i].hash);
